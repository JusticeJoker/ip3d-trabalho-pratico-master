...
namespace TankGame
{
    class EnemyBullet
    {
        ...
        public EnemyBullet(GraphicsDevice device, ContentManager content)
        {
        ...
        public void Update(GameTime gameTime, KeyboardState kb, Terrain terrain, Tank tank, EnemyTank enemyTank)
        {
            ...
            {
                isAlive = true;
                position = enemyTank.boneTransforms[10].Translation;
                speed = enemyTank.boneTransforms[10].Backward;
                speed.Normalize();
            ...
            if (isAlive)
            {
                distance = position - tank.position;
